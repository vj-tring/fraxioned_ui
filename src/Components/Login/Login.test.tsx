...
describe('Login Component', () => {
  ...
  test('renders login form', () => {
    ...
    expect(
      screen.getByText('Please enter your details to sign in')
    ).toBeInTheDocument()
  ...
  test('displays error for invalid email', () => {
    ...
    expect(
      screen.getByText('Please enter a valid email id')
    ).toBeInTheDocument()
  })
  ...
  test('initial state is correct', () => {
    ...
    expect(
      screen.queryByText('Please fill in the Email ID')
    ).not.toBeInTheDocument()
    expect(
      screen.queryByText('Please fill in the Password')
    ).not.toBeInTheDocument()
  ...
  test('clears email error on valid input', () => {
    renderLogin()
    const emailInput = screen.getByPlaceholderText('Email')
    fireEvent.change(emailInput, { target: { value: 'invalidemail' } })
    fireEvent.blur(emailInput)
  ...
  test('displays email error when only password is provided', () => {
    renderLogin()
    const passwordInput = screen.getByPlaceholderText('Password')
    fireEvent.change(passwordInput, { target: { value: 'password123' } })
    const submitButton = screen.getByRole('button', { name: /Sign in/i })
  ...
  test('applies error styling to inputs', () => {
    renderLogin()
    const emailInput = screen.getByPlaceholderText('Email')
    const passwordInput = screen.getByPlaceholderText('Password')
    const submitButton = screen.getByRole('button', { name: /Sign in/i })
    ...
    fireEvent.click(submitButton)
    expect(emailInput).toHaveClass('errorInput')

    fireEvent.change(emailInput, { target: { value: 'test@example.com' } })
    fireEvent.click(submitButton)
    expect(passwordInput).toHaveClass('errorInput')
  })

  test('displays error for missing email when password is entered', () => {
    renderLogin()
    const passwordInput = screen.getByPlaceholderText('Password')
    const submitButton = screen.getByRole('button', { name: /Sign in/i })
  ...
  test('displays error message on login failure', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Login failed|An error occurred/)
      ).toBeInTheDocument()
    })
  })
  ...
  test('submits form with valid inputs', async () => {
    ...
    await waitFor(() => {
      expect(mockedAxios.post).toHaveBeenCalledWith(
        'http://mock-api-url.com/authentication/login',
        { email: 'test@example.com', password: 'password123' }
      )
    })
  })

  test('email input is focused on render', () => {
    renderLogin()
    const emailInput = screen.getByPlaceholderText('Email')
    expect(emailInput).toHaveFocus()
  })
