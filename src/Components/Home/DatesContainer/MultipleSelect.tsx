...
const MultipleSelect: React.FC = () => {
  ...
  const handleCountChange = (name: string, action: 'increase' | 'decrease') => {
    setCounts(prevCounts => ({
      ...prevCounts,
      [name]: action === 'increase' ? prevCounts[name] + 1 : Math.max(prevCounts[name] - 1, 0),
    }));
  };
        ...
        {names.map((item) => (
          ...
          disableRipple>
            <div className='d-flex justify-content-between w-100 MultiItems'>
              <Avatar sx={{ backgroundColor: '#df9526', marginRight: '12px' }}>{item.icon}</Avatar>
              <div className=' w-50 '>
                  <b>{item.label}</b>
              ...
              <div className='d-flex justify-content-around w-50 pb-2'>
                ...
                <button className="Inc-circle" onClick={() => handleCountChange(item.label, 'increase')}>+</button>
              </div>
            </div>
          </MenuItem>
        ))}
