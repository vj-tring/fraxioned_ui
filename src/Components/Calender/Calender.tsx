...
import { ORIENTATION, StatefulCalendar as StatefulCalendarType } from 'baseui/datepicker';
...
const Calendar: React.FC = () => {
  ...
  const getFirstUnavailableDateAfterCheckIn = (checkInDate: Date) => {
    return unavailableDates.find((unavailableDate) => unavailableDate > checkInDate);
  };

  const getLastCheckOutDate = (checkInDate: Date) => {
    const sortedBookedDates = bookedDates.slice().sort((a, b) => b.getTime() - a.getTime());
  ...
  const handleDateChange = ({ date }: { date: Date | Date[] }) => {
    if (Array.isArray(date)) {
      ...
      if (isDayBeforeBookedDate(start)) {
        ...
        const nightsSelected = (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24);
        ...
        const minNightsBetween = lastCheckOutDate
          ? (start.getTime() - lastCheckOutDate.getTime()) / (1000 * 60 * 60 * 24)
          : null;

        if (lastCheckOutDate && minNightsBetween < 5) {
          setErrorMessage('Minimum 5 nights required between previous check-out and current check-in');
        } else if (lastMinute) {
          if (nightsSelected < 1 || nightsSelected > 3) {
            setErrorMessage('Minimum 1 night is required for Last-Minute Booking');
          } else {
...
<div className="card-item " onClick={handleClick}>
          ...
          <StatefulCalendar
            ...
            filterDate={(date) => {
              if (startDate && !endDate) {
                const firstBookedDate = getFirstBookedDateAfterCheckIn(startDate);
                ...
                return (
                  ...
                  (!lastMinute || (lastMinute && date <= new Date(startDate.getTime() + 3 * 24 * 60 * 60 * 1000)))
                );
              }
              return (
                date >= today &&
            ...
            overrides={{
              Day: {
                style: ({ $date }) => {
                  ...
                  const firstBookedDate = startDate && !endDate ? getFirstBookedDateAfterCheckIn(startDate) : null;
                  const firstUnavailableDate = startDate && !endDate ? getFirstUnavailableDateAfterCheckIn(startDate) : null;
                  ...
                  } else if (startDate && firstBookedDate && $date > firstBookedDate) {
                    return {
                      color: 'gray',
                      textDecoration: 'line-through',
                      pointerEvents: 'none',
                    };
                  } else if (startDate && firstUnavailableDate && $date > firstUnavailableDate) {
                    return {
                      color: 'gray',
                      pointerEvents: 'none',
                      textDecoration: 'line-through',
      ...
      <div className='stay-length '>
        ...
        <div className='season'>Peak-Season Nights: June 1 - September 30</div>
