...
import {
    createNewSpace, fetchAllSpaces, updateExistingSpace,
    deleteExistingSpace, Space
...
const Spaces: React.FC<{ isSidebarOpen: boolean }> = ({ isSidebarOpen }) => {
    ...
    const handleAddSpace = async () => {
        try {
            ...
            if (selectedSpace) {
                await dispatch(updateExistingSpace({ id: selectedSpace.id!, spaceData: formData })).unwrap();
            } else {
    ...
    const resetForm = () => {
        setFormValues({ name: '', isBedTypeAllowed: false, isBathroomTypeAllowed: false, imageFile: null });
        ...
        <div className={`${styles.usersContainer} ${isSidebarOpen ? styles.sidebarOpen : styles.sidebarClosed}`}>
            <div className={styles.fullContainer}>
                ...
                <div className={styles.mainsection}>
                    <div className={`${styles.spaceList} ${(!spaces || spaces.length === 0) ? styles.noRooms : ''}`}>
                                ...
                                <div key={index} className={styles.spaceItem}>
                                    <div className={styles.spaceCard}>
                                        ...
                                        <div className={styles.spaceNameRow}>
                                            <p>{space.name}</p>
                                            <Button onClick={() => handleEditSpace(space)}>Edit</Button>
                                            <Button color="secondary" onClick={() => confirmDeleteSpace(space)}>Delete</Button>
                                        </div>
                                    </div>
                                </div>
                            ))
                ...
                <Dialog open={isDialogOpen} onClose={handleDialogClose} fullWidth maxWidth='sm'>
                    <DialogTitle>{selectedSpace ? "Edit Space" : "Create Space"}</DialogTitle>
                    <DialogContent sx={{ gap: 3, display: 'flex', flexDirection: 'column' }}>
                        ...
                        <Box>
                            <FormControlLabel
                                control={<Checkbox checked={formValues.isBedTypeAllowed} onChange={() => setFormValues({ ...formValues, isBedTypeAllowed: !formValues.isBedTypeAllowed })} />}
                                label="Is Bed Type Allowed"
                            />
                            <FormControlLabel
                                control={<Checkbox checked={formValues.isBathroomTypeAllowed} onChange={() => setFormValues({ ...formValues, isBathroomTypeAllowed: !formValues.isBathroomTypeAllowed })} />}
                    ...
                    <DialogActions>
                        ...
                        <Button onClick={handleAddSpace} color="primary">{selectedSpace ? "Update" : "Save"}</Button>
                    </DialogActions>
                </Dialog>
                ...
                <Dialog open={isDeleteDialogOpen} onClose={() => setIsDeleteDialogOpen(false)}>
                    ...
                    <DialogContent>Are you sure you want to delete {selectedSpace?.name}?</DialogContent>
                    <DialogActions>
                        <Button onClick={() => setIsDeleteDialogOpen(false)} color="primary">Cancel</Button>
                        <Button onClick={() => handleDeleteSpace(selectedSpace!)} color="secondary">Delete</Button>
                    </DialogActions>
                </Dialog>
            </div>
        </div>
