...
export default function Component({ initialSpace = {} }) {
  ...
  >([]);
  const [showBedTypesTab, setShowBedTypesTab] = useState<boolean>(false);
  const [showBathTypesTab, setShowBathTypesTab] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetchBedBathType = async () => {
      const res = spaceData.find((data) => data.id === space.spaceId)
      if (res.isBedTypeAllowed) {
        setShowBedTypesTab(true);
      }
      if (res.isBathroomTypeAllowed) {
        setShowBathTypesTab(true);
      }
    }
    fetchBedBathType();
  }, [spaceData])
