...
import { PropertyAmenityGroup } from "./interface";
...
export default function Component({ initialSpace = {} }) {
  ...
  const { amenities: propertyAmenities, loading: amenitiesLoading, error: amenitiesError } = useSelector(
    (state: RootState) => state.propertyAmenities
  );
  const [photos, setPhotos] = useState<File[]>([]);
  const [photoPreviews, setPhotoPreviews] = useState<string[]>([]);
  const [amenityGroups, setAmenityGroups] = useState<PropertyAmenityGroup[]>([]);
  ...
  useEffect(() => {
    if (propertyAmenities.length > 0) {
      const groupedAmenities = propertyAmenities.reduce((acc: PropertyAmenityGroup[], item) => {
    ...
    setAmenityGroups((prev) =>
      prev.map((group) =>
        group.name === category
          ? {
              ...group,
              amenities: group.amenities.filter((a) => a !== amenity),
            }
          : group
      )
    );
                ...
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {selectedCategory &&
                        amenityGroups
                          .find((group) => group.name === selectedCategory)
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        removeAmenity(group.name, amenity)
                                      }
