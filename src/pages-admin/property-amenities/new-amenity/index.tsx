...
const NewAmenityForm: React.FC<NewAmenityFormProps> = ({
  onClose,
  onAmenityAdded,
}) => {
      ...
      {previewUrl ? (
        <div className={styles.previewContainer}>
          ...
          <div className={styles.fileInfo}>
            <span className={styles.fileName}>{formData.imageFile?.name}</span>
  ...
  const handleAddCustomAmenityType = useCallback(async () => {
    ...
    if (
      customAmenityType &&
      !amenityGroups?.some((group) => group.name === customAmenityType)
    ) {
      await dispatch(
        createAmenityGroup({ createdBy: { id: 1 }, name: customAmenityType })
      );
    }
    setFormData((prev) => ({
      ...prev,
  ...
  const renderAmenityGroupSelect = () => (
    <div className={styles.amenityTypeContainer}>
      <select
        ...
        onChange={(e) => {
          const selected = amenityGroups?.find(
            (group) => group.id === Number(e.target.value)
          );
          setFormData((prev) => ({
            ...prev,
            selectedAmenityGroup: selected || null,
          }));
        }}
      ...
      {!formData.showCustomAmenityInput && (
        <button
          ...
          onClick={() =>
            setFormData((prev) => ({ ...prev, showCustomAmenityInput: true }))
          }
        >
  ...
  const renderCustomAmenityInput = () => (
    <div className={styles.customAmenityContainer}>
      ...
      <button
        ...
        className={styles.iconButton}
        onClick={() =>
          setFormData((prev) => ({ ...prev, showCustomAmenityInput: false }))
        }
      >
        <AiOutlineClose />
      </button>
      ...
      <div className={styles.formContainer}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          <div className={styles.formGroup}>
            <label htmlFor="amenityGroup" className={styles.label}>
              Amenity Group*
            </label>
          ...
          <div className={styles.formGroup}>
            <label htmlFor="amenityName" className={styles.label}>
              Amenity Name*
            </label>
            <input
              type="text"
          ...
          <div className={styles.formGroup}>
            <label htmlFor="amenityDescription" className={styles.label}>
