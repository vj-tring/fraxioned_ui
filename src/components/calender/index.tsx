...
export function DatePickerWithRange({
  ...
  const meetsConsecutiveStayRule = (startDate: Date, endDate: Date) => {
    ...
    for (const booking of userBookings) {
      ...
      // Calculate days since previous checkout date for the end date
      ...
      // Calculate days before next check-in date for the start date
      const daysBeforeCheckinStart = Math.floor(
        (checkInDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)
      );
  
      // Calculate days before next check-in date for the end date
