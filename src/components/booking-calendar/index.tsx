...
const getBookedDatesForProperty = (propertyId: number) => {
  ...
  return userBookings
    ...
    .flatMap((booking: any) => {
      ...
      end.setDate(end.getDate() + 1); // Extend the end date by one day
      const dates = [];
    ...
    const renderPropertyCalendar = (property: any, isLast: boolean, isOdd: boolean) => (
      ...
      >
        <h3 style={{ color: property.color, textAlign: 'left', marginLeft: '20px', marginTop: '10px'}}>
          {property.name}
        </h3>
        <Calendar
          mode="range"
          ...
          modifiers={{
            ...
            bookedStart: (date) => {
              const bookedDates = getBookedDatesForProperty(property.id);
              return bookedDates.some((bookedDate, index) => 
                date.toDateString() === bookedDate.toDateString() && 
                (index === 0 || bookedDates[index - 1].toDateString() !== new Date(bookedDate.getTime() - 86400000).toDateString())
              );
            },
            bookedEnd: (date) => {
              const bookedDates = getBookedDatesForProperty(property.id);
              return bookedDates.some((bookedDate, index) => 
                date.toDateString() === bookedDate.toDateString() && 
                (index === bookedDates.length - 1 || bookedDates[index + 1].toDateString() !== new Date(bookedDate.getTime() + 86400000).toDateString())
              );
            },
          }}
          modifiersStyles={{
            ...
            bookedStart: {
              color: 'white',
              // backgroundColor: property.color,
              background: `linear-gradient(-45deg, ${property.color} 50%, white 50%)`,
              position: 'relative',
              overflow: 'hidden',   
            },
            bookedEnd: {
              ...
              position: 'relative',
              overflow: 'hidden',
              background: `linear-gradient(135deg, ${property.color} 50%, white 50%)`,
            },
          }}
          modifiersClassNames={{
            booked: 'booked-date',
            bookedStart: 'booked-date-start',
            bookedEnd: 'booked-date-end',
          }}
