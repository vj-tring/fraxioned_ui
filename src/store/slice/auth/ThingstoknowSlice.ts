...
import { getProperrtDetailsbyId } from '@/api/api-endpoints';
...
export const fetchPropertyDetailsById = createAsyncThunk(
  ...
  async (propId: number, { rejectWithValue }) => {
    try {
      const response = await getProperrtDetailsbyId(propId);
...
const ThingsToKnowSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPropertyDetailsById.fulfilled, (state, action: PayloadAction<any>) => {
        state.propertyDetails = action.payload;
        state.loading = false;
      })
      .addCase(fetchPropertyDetailsById.rejected, (state, action: PayloadAction<any>) => {
        state.loading = false;
...
export const selectPropertyDetails = (state: RootState) => state.thingsToKnow.propertyDetails;
